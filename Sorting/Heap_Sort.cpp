...
unsigned long int heap_sort_comparison_counter = 0;
unsigned long int heap_sort_key_swap_counter = 0;
...
void heapSort(std::vector<int>& v, Order order, std::unique_ptr<Result>& r)
{
    ...
    r -> type = toolBox::HEAP;
    r -> comparisons = heap_sort_comparison_counter;
    r -> swaps = heap_sort_key_swap_counter;
    r -> time = time_taken;
